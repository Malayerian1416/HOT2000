...
const buttonLoading = (button, state) => {
    switch (state){
        case "load":{
            button.disabled = true;
            button.childNodes[0].classList.remove("sr-only");
            button.childNodes[1].classList.add("sr-only");
            break;
        }
        case "stop":{
            button.disabled = false;
            button.childNodes[0].classList.add("sr-only");
            button.childNodes[1].classList.remove("sr-only");
            break;
        }
        default:{
            button.disabled = false;
...
const filterTables = (inputElement) => {
    let searchQuery = inputElement.value.toLowerCase();
    let table = document.getElementById(inputElement.dataset.table), columns, tr, td, i, j, txtValue;
    columns = JSON.parse(table.dataset.columns);
    tr = table.getElementsByTagName("tr");
    for (i = 1; i < tr.length; i++) {
        let strings = [];
        for (j = 0; j < columns.length; j++) {
            td = tr[i].getElementsByTagName("td")[parseInt(columns[j])] || tr[i].getElementsByTagName("th")[parseInt(columns[j])];
            if (td) {
                txtValue = td.innerHTML !== '' ? td.querySelector('input') !== null ? td.querySelector('input').value.toLowerCase() : td.textContent.toLowerCase() || td.innerText.toLowerCase() : null;
                if (txtValue)
        ...
        if (strings.length) {
            const match = strings.find(element => {
                ...
                return !!clearElement.includes(searchQuery);
            });
            if (match)
                tr[i].style.display = "";
            else
...
<template>
    ...
    <alerts v-if="alert.show"></alerts>
    <div class="antialiased bg-gray-50 dark:bg-gray-900">
        <nav id="nav" class="bg-white border-b border-gray-200 px-4 py-2.5 dark:bg-gray-800 dark:border-gray-700 fixed left-0 right-0 top-0 z-50">
        ...
        <main id="main" class="md:ml-64 h-auto">
